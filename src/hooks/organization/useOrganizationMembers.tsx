...
export const useOrganizationMembers = (organizationId: string | undefined): UseOrganizationMembersReturn => {
  ...
  const fetchMembers = async () => {
    ...
    try {
      ...
      const transformedData: OrganizationMember[] = data.map(item => {
        // Handle potential SelectQueryError case
        let userData = null;
        if (item.user && !('error' in item.user)) {
          userData = item.user;
        } else if (item.invited_email) {
          // Create user data from invited info
          userData = {
  ...
  const inviteMember = async (email: string, name: string, role: UserRole): Promise<OrganizationMember | null> => {
    ...
    try {
      // Cast to any to work around TypeScript limitations with Supabase
      const memberData = {
